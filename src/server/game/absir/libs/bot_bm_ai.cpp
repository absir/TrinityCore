...
public:
    ...
    {
		if (bot_minion_ai* ai = AB_GetBotMinionAI(creature))
    ...
    private:
        ...
        class DelayedMeleeDamageEvent : public BasicEvent
        {
            ...
            protected:
                ...
                {
                    bool suc = false;
					if (Creature* bot = sObjectAccessor->GetObjectInOrOutOfWorld((ObjectGuid)_botGuid, (Creature*)NULL))
                    {
                        ASSERT(AB_GetBotClass(bot) == BOT_CLASS_BM);
                        (dynamic_cast<blademaster_bot::blademaster_botAI*>(AB_GetBotMinionAI(bot)))->CriticalStrikeFinish(_targetGuid, _dinfo, _windwalk);
                        suc = true;
                    }
        ...
        class EventTerminateEvent : public BasicEvent
        {
            ...
            protected:
                bool Execute(uint64 /*e_time*/, uint32 /*p_time*/)
                {
                    if (Creature* bot = sObjectAccessor->GetObjectInOrOutOfWorld((ObjectGuid)_botGuid, (Creature*)NULL))
                    {
						(dynamic_cast<blademaster_bot::blademaster_botAI*>(AB_GetBotMinionAI(bot)))->TerminateEvent();
                        return true;
                    }
                    return false;
                }
        ...
        class DelayedIllusionSummonEvent : public BasicEvent
        {
            ...
            protected:
                bool Execute(uint64 /*e_time*/, uint32 /*p_time*/)
                {
                    ...
                    {
                        ASSERT(AB_GetBotClass(bot) == BOT_CLASS_BM);
                        (dynamic_cast<blademaster_bot::blademaster_botAI*>(AB_GetBotMinionAI(bot)))->MirrorImageFinish();
                        return true;
                    }
        ...
        class DisappearEvent : public BasicEvent
        {
            ...
            protected:
                bool Execute(uint64 /*e_time*/, uint32 /*p_time*/)
                {
                    ...
                    {
                        ASSERT(AB_GetBotClass(bot) == BOT_CLASS_BM);
                        (dynamic_cast<blademaster_bot::blademaster_botAI*>(AB_GetBotMinionAI(bot)))->MirrorImageMid();
                        return true;
                    }
    ...
    public:
        ...
        void MirrorImageStart()
        {
            ...
            while (int8(_illusionGuids.size()) > (3 - illusionsCount))
            {
                ...
                if (Creature* illusion = sObjectAccessor->GetObjectInWorld((ObjectGuid)*itr, (Creature*)NULL))
                    if (!illusion->IsDuringRemoveFromWorld())
						AB_GetBotAI(illusion->ToCreature())->JustDied(NULL);
        ...
        void MirrorImageFinish()
        {
            ...
            for (uint8 i = 0; i != illusionsCount; ++i)
            {
                ...
                AB_GetBotAI(illusion)->SetBotIsTemp();
                AB_GetBotAI(illusion)->FindMaster(true);
                ...
                AB_SetBotCommandState(illusion, COMMAND_ABANDON);

                _illusionGuids.push_back(illusion->GetGUID());
            }

            AB_SetBotCommandState(me, COMMAND_ABANDON);
        ...
        void CriticalStrikeFinish(uint64 targetGuid, CalcDamageInfo* dinfo, bool /*windwalk*/)
        {
            ...
            Unit* target = sObjectAccessor->GetUnit(*me, (ObjectGuid)targetGuid);
        ...
        void UnsummonAll()
        {
            while (!_illusionGuids.empty())
            {
                std::list<uint64>::iterator itr = _illusionGuids.begin();
                if (Creature* illusion = sObjectAccessor->GetObjectInWorld((ObjectGuid)*itr, (Creature*)NULL))
                    if (AB_GetBotAI(illusion))
                        AB_GetBotAI(illusion)->JustDied(NULL);
