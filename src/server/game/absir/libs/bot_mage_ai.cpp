...
public:
    ...
    struct mage_botAI : public bot_minion_ai
    {
        ...
        void CheckBlink(uint32 diff)
        {
			...
			if (!me->IsInCombat() && me->GetExactDist2d(master) > std::max<uint8>(AB_GetBotFollowDist(master), 35) &&
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!target)
                    {
                        ...
                        {
                            Player* pPlayer = itr->GetSource();
                            if (!pPlayer || !pPlayer->IsInWorld() || !AB_HaveBot(pPlayer)) continue;
                            if (me->GetMapId() != pPlayer->GetMapId()) continue;
                            BotMap const* map = AB_GetBotMap(pPlayer);
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            if (aftercastTargetGuid != 0)
            {
				...
				if (!IS_PLAYER_GUID(((ObjectGuid)aftercastTargetGuid)))
                {
                    aftercastTargetGuid = 0;
                    return;
                }

				Player* pTarget = sObjectAccessor->FindPlayer((ObjectGuid)aftercastTargetGuid);
                aftercastTargetGuid = 0;
