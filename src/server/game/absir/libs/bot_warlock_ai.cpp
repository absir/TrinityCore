...
public:
    ...
    struct warlock_botAI : public bot_minion_ai
    {
		...
		void JustDied(Unit* u) { AB_SetBotsPetDied(me); bot_minion_ai::JustDied(u); }
...
public:
    ...
    struct voidwalker_botAI : public bot_pet_ai
    {
		...
		void JustDied(Unit*) { AB_SetBotsPetDied(m_creatureOwner); }
        void DoNonCombatActions() { }
        ...
        void DamageTaken(Unit* u, uint32& /*damage*/)
        {
            if (m_creatureOwner->IsAIEnabled)
				if (bot_minion_ai* ai = AB_GetBotMinionAI(m_creatureOwner))
