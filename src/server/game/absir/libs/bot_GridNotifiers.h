...
class PolyUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            if (!_botPvP && !(me->ToCreature() && AB_IsFreeBot(me->ToCreature())) && u->IsControlledByPlayer())
                return false;
            if (u == mytar)
                return false;
            if (!me->IsWithinDistInMap(u, m_range))
            ...
            if (me->GetDistance(u) < 6 || mytar->GetDistance(u) < 5 ||
                (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_MAGE && u->GetHealthPct() < 70))
                return false;
            if (!u->InSamePhase(me))
                return false;
            ...
            if (!u->IsVisible())
                return false;
            if (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_MAGE ? !u->getAttackers().empty() : u->getAttackers().size() > 1)
                return false;
            if (!u->IsHostileTo(me))
                return false;
            if (u->IsPolymorphed() ||
            ...
            if (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_MAGE && !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(118)))//Polymorph
                return true;
            if (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_SHAMAN && !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(51514)))//Hex
                return true;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (u->GetCreatureType() != CREATURE_TYPE_BEAST &&
                AB_GetBotClass(me->ToCreature()) == BOT_CLASS_HUNTER)
                return false;
            if (!u->IsAlive())
                return false;
            if (!u->isTargetableForAttack())
            ...
            if (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_WARLOCK &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(5782)))//fear rank1
                return true;
            if (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_HUNTER &&
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->GetTypeId() == TYPEID_UNIT)
				if (Player* mymaster = AB_GetBotOwner(me->ToCreature()))
            ...
            if (u->HasAuraType(SPELL_AURA_PERIODIC_DAMAGE))
                return false;
            if (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_PALADIN &&
                ...
                return false;
            if (AB_GetBotClass(me->ToCreature()) == BOT_CLASS_HUNTER && u->isFeared())
                return false;
...
class CastingUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            if (!m_friend && !_botPvP && !(me->ToCreature() && AB_IsFreeBot(me->ToCreature())) && u->IsControlledByPlayer())
                return false;
