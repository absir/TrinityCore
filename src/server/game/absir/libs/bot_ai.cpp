...
{
    m_creatureOwner = AB_GetCreatureOwner(me);
    basearmor = 0;
}
...
{
	return IAmFree() ? urand(0, 100) : urand(0, 100 + (AB_GetNpcBotsCount(master) - 1) * 10);
}
...
void bot_ai::BotWhisper(char const* text, Player const* target) const
{
	...
	me->Whisper(text, LANG_UNIVERSAL, player, false);
}
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, bool triggered, uint64 originalCaster)
{
    ...
    if (me->isMoving() && m_botSpellInfo->CalcCastTime() > 0)
        AB_BotStopMovement(me);

    TriggerCastFlags flags = triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE;
    SpellCastTargets targets;
    targets.SetUnitTarget(victim);
    Spell* spell = new Spell(me, m_botSpellInfo, flags, (ObjectGuid)originalCaster);
...
void bot_minion_ai::_calculatePos(Position& pos)
{
    ...
    uint8 followdist = AB_GetBotFollowDist(master);
    float mydist, angle;
...
void bot_minion_ai::BuffAndHealGroup(Player* gPlayer, uint32 diff)
{
    ...
    if (IAmFree())
    {
        ...
        if (BuffTarget(me, diff))
            return;
        if (Creature* pet = AB_GetBotsPet(me))
        {
        ...
        {
			if (Unit* u = sObjectAccessor->GetUnit(*me, (ObjectGuid)*itr))
            {
    ...
    if (!pGroup)
    {
        ...
        for (Unit::ControlList::const_iterator itr = master->m_Controlled.begin(); itr != master->m_Controlled.end(); ++itr)
        {
            ...
            if (Creature* cre = u->ToCreature())
                if (AB_IS_BOT_UNIT(cre) || cre->IsPet())
                    if (BuffTarget(u, diff))
                        return;
        }
        return;
    ...
    if (HasRole(BOT_ROLE_HEAL))
    {
        for (uint8 i = 0; i != TARGETICONCOUNT; ++i)
        {
            if (_healTargetIconFlags & GroupIconsFlags[i])
            {
                ...
                {
					if (Unit* unit = sObjectAccessor->FindUnit((ObjectGuid)guid))
                    {
...
void bot_minion_ai::CureGroup(Player* pTarget, uint32 cureSpell, uint32 diff)
{
    ...
    if (!pGroup)
    {
        ...
        BotMap const* map = AB_GetBotMap(master);
    ...
    else
    {
        ...
        {
            Player* tPlayer = itr->GetSource();
            if (!tPlayer || (tPlayer->isDead() && !AB_HaveBot(tPlayer))) continue;
            if (!Bots && AB_HaveBot(tPlayer))
                Bots = true;
...
void bot_ai::_listAuras(Player* player, Unit* unit) const
{
    ...
    {
		if (AB_IS_BOT_UNIT(unit))
        {
            botstring << "minion bot, master: ";
			Player* owner = AB_GetBotAI(unit->ToCreature())->GetBotOwner();
            botstring << (owner != unit ? owner->GetName() : "none");
        }
		else if (AB_IS_BOT_PET_UNIT(unit))
        {
			Player* owner = AB_GetBotAI(unit->ToCreature())->GetBotOwner();
			Creature* creowner = AB_GetBotPetAI(unit->ToCreature())->GetCreatureOwner();
	...
	uint32 const bot_pet_player_class = unit->GetTypeId() == TYPEID_PLAYER ? unit->getClass() : AB_GetBotAI(unit->ToCreature())->GetBotClass();
    ...
    if (unit == me)
    {
        ...
        if (!IAmFree())
            ch.PSendSysMessage("Follow distance: %u", AB_GetBotFollowDist(master));
...
void bot_minion_ai::SetStats(bool force, bool shapeshift)
{
    ...
    //SetStats for pet
    if (Creature* pet = AB_GetBotsPet(me))
        if (bot_pet_ai* petai = AB_GetBotPetAI(pet))
...
void bot_pet_ai::SetStats(bool force, bool /*unk*/)
{
    ...
    {
		switch (AB_GetBotClass(m_creatureOwner))
        {
            case BOT_CLASS_WARLOCK:
				value = float(AB_GetBotAI(m_creatureOwner)->GetBotSpellPower());
                break;
            case BOT_CLASS_DEATH_KNIGHT:
            ...
            default: //some weird class or NYI
				TC_LOG_ERROR("entities.player", "*etStats():Damage - unknown bot owner class %u", uint8(AB_GetBotClass(m_creatureOwner)));
                value = 0.0f;
                break;
        }
    ...
    //DAMAGE TAKEN
	dmg_taken = AB_GetBotAI(m_creatureOwner)->GetBotDamageTakenMod();
	...
	haste = AB_GetBotAI(m_creatureOwner)->GetHaste();
    ...
    //HIT
	hit = AB_GetBotAI(m_creatureOwner)->GetHitRating();
    ...
    //MANA REGEN
	regen_mp = AB_GetBotAI(m_creatureOwner)->GetManaRegen();

    //SPELL PENETRATION
	spellpen = AB_GetBotAI(m_creatureOwner)->GetBotSpellPenetration();
    ...
    if (spp_mod > 0.f)
    {
        ...
        {
            case BOT_CLASS_WARLOCK:
				value = AB_GetBotAI(m_creatureOwner)->GetBotSpellPower();
                break;
            case BOT_CLASS_DEATH_KNIGHT:
                value = m_creatureOwner->GetTotalAttackPowerValue(BASE_ATTACK);
                break;
            case BOT_CLASS_HUNTER:
                ...
                break;
            default: //some weird class or NYI
				TC_LOG_ERROR("entities.player", "*etStats():Spellpower - unknown bot owner class %u", uint8(AB_GetBotClass(m_creatureOwner)));
                value = 0.f;
                break;
        }
...
bool bot_ai::IsInBotParty(Unit const* unit) const
{
    ...
    if (IAmFree())
    {
        ...
        {
            //controlled bot case
			if (AB_GetBotAI(bot) && !AB_IsFreeBot(bot))
				owner = AB_GetBotOwner(bot);
    ...
    if (Creature const* cre = unit->ToCreature())
    {
		...
		if (Player* owner = AB_GetBotOwner(cre))
        {
            if (owner == master)
                return true;
        }
        ...
        else
        {
            ...
            if (Group* gr = master->GetGroup())
                if (gr->IsMember((ObjectGuid)ownerGuid))
                    return true;
        }
    }
...
//CHECKAURASAB_GetNpcBotsCount(master)
...
void bot_minion_ai::CheckAuras(bool force)
{
    ...
    {
        checkAurasTimer = 10 + (IAmFree() ? 5 : AB_GetNpcBotsCount(master) / 2);

        if (needparty)
        {
            needparty = false;
			if (!IAmFree()) //we could lose master
				AB_AddBotToGroup(master, me);
        }
...
bool bot_ai::CanBotAttack(Unit const* target, int8 byspell) const
{
    ...
    if (!_botPvP && !IAmFree() && target->IsControlledByPlayer())
        return false;
    uint8 followdist = IAmFree() ? 100 : AB_GetBotFollowDist(master);
...
Unit* bot_ai::_getTarget(bool byspell, bool ranged, bool &reset) const
{
    ...
    if (!mytar && IsMinionAI())
        if (Creature* pet = AB_GetBotsPet(me))
            mytar = pet->GetVictim();
    if (!mytar && IsPetAI())
        mytar = AB_GetCreatureOwner(me)->GetVictim();
    ...
    if (!IAmFree())
    {
        ...
        if (!gr)
        {
            ...
            for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
            {
				...
				Creature* pet = NULL;
				if (AB_IS_BOT_UNIT(bot)) {
					pet = AB_GetBotsPet(bot);
				}
        ...
        else
        {
            for (GroupReference* ref = gr->GetFirstMember(); ref != NULL; ref = ref->next())
            {
				...
				if (!AB_HaveBot(pl)) continue;
				BotMap const* map = AB_GetBotMap(pl);
                for (BotMap::const_iterator it = map->begin(); it != map->end(); ++it)
                {
...
bool bot_ai::InDuel(Unit const* target) const
{
	...
	bool isbot = target->GetTypeId() == TYPEID_UNIT && AB_GetBotAI(target->ToCreature());
	Player const* player = target->GetTypeId() == TYPEID_PLAYER ? target->ToPlayer() : isbot ? AB_GetBotOwner(target->ToCreature())->ToPlayer() : NULL;
    if (!player)
    {
...
Unit* bot_minion_ai::FindAOETarget(float dist, bool checkbots, bool targetfriend) const
{
    ...
    for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        Player* tPlayer = itr->GetSource();
        if (!tPlayer) continue;
        if (checkbots && AB_HaveBot(tPlayer))
            Bots = true;
        if (!tPlayer->IsInWorld() || tPlayer->IsBeingTeleported()) continue;
...
void bot_minion_ai::Evade(bool force)
{
    ...
    Creature* m_botsPet = AB_GetBotsPet(me);
    ...
    if (master->IsInCombat() && !IAmFree())
    {
        if (!master->getHostileRefManager().isEmpty())
        {
            ...
            {
				Unit* unit = sObjectAccessor->FindUnit((ObjectGuid)*i);
    ...
    if (!master->IsInCombat() || IAmFree())
    {
        ...
        for (BotMap::const_iterator it = map->begin(); it != map->end(); ++it)
        {
            ...
            Creature* m_botsPet = AB_GetBotsPet(cre);
...
uint8 bot_ai::GetWait()
{
    ...
    return IAmFree() ? 3 : (1 + (AB_GetNpcBotsCount(master) - 1)/3 + (irand(0,100) <= 50)*int8(RAND(-1,1)));
}
//Damage/Healing Mods
...
bool bot_minion_ai::OnGossipHello(Player* player, Creature* creature, uint32 /*option*/)
{
    ...
    if (!_enableNpcBots || creature->IsInCombat() || bot_ai::CCed(creature) || AB_GetBotAI(creature)->IsDuringTeleport())
    {
        player->CLOSE_GOSSIP_MENU();
        return true;
    }

    if (AB_GetBotAI(creature)->IsTempBot()) //Blademaster illusion etc.
    {
        player->CLOSE_GOSSIP_MENU();
        return true;
    }
    ...
    uint32 gossipTextId = (player->GetGUIDLow() == AB_GetBotAI(creature)->GetBotOwnerGuid() || !AB_GetBotAI(creature)->IAmFree()) ? GOSSIP_SERVE_MASTER : GOSSIP_NEED_SMTH;

    bool menus = false;

    if (player->IsGameMaster() &&
        (AB_IsFreeBot(creature) || player->GetGUIDLow() != AB_GetBotAI(creature)->GetBotOwnerGuid()))
    {
	...
	/*
    if (player->GetGUIDLow() != AB_GetBotAI(creature)->GetBotOwnerGuid())
    {
        if (AB_IsFreeBot(creature) && !player->IsGameMaster())
        {
            ...
            if (creature->IsHostileTo(player) || player->IsHostileTo(creature) ||
                ...
                reason = -1;
            if (!reason && AB_GetBotAI(creature)->GetBotOwnerGuid())
                reason = 1;
            if (!reason && AB_GetNpcBotsCount(player) >= BotMgr::GetMaxNpcBots())
                reason = 2;
            if (!reason && !player->HasEnoughMoney(cost))
                reason = 3;

            if (!reason && _maxClassNpcBots && AB_HaveBot(player))
            {
                uint8 count = 0;
                BotMap const* map = player->GetBotMgr()->GetBotMap();
                for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
                    if (itr->second->GetBotClass() == AB_GetBotClass(creature))
                        ++count;
            ...
            if (AB_GetBotClass(creature) >= BOT_CLASS_EX_START)
	...
	*/
    ...
    if (AB_GetBotAI(creature)->GetBotOwnerGuid())
    {
        Group const* gr = player->GetGroup();

        if (player == AB_GetBotOwner(creature))
        {
        ...
        if (player == AB_GetBotOwner(creature) || (gr && gr->IsMember(AB_GetBotOwner(creature)->GetGUID())))
        {
            switch (AB_GetBotClass(creature))
            {
                case BOT_CLASS_MAGE:
    ...
    switch (sender)
    {
		...
		*/
        case GOSSIP_SENDER_FORMATION:
        {
			...
			diststr << "Set distance (current: " << uint32(AB_GetBotFollowDist(player)) << ')';
...
bool bot_minion_ai::OnGossipSelectCode(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action, char const* code)
{
    ...
    switch (sender)
    {
        case GOSSIP_SENDER_FORMATION_DISTANCE:
        {
            ...
            AB_SetBotFollowDist(player, distance);
...
void bot_minion_ai::SummonBotsPet(uint32 entry)
{
    ...
    */
	...
	AB_SetBotOwner(m_botsPet, master);
	AB_SetCreatureOwner(m_botsPet, me);
...
void bot_minion_ai::OnOwnerDamagedBy(Unit* attacker)
{
	...
	float maxdist = InitAttackRange(float(IAmFree() ? 100 : AB_GetBotFollowDist(master)), ranged); //use increased range
...
void bot_minion_ai::InitRoles()
{
    ...
    stmt->setUInt32(0, me->GetEntry());
	*/
	PreparedQueryResult result = NULL; //CharacterDatabase.Query(stmt);
    if (!result)
    {
        //default roles
        _roleMask = BOT_ROLE_DPS;
...
bool bot_ai::IsTank(Unit* unit) const
{
    ...
    if (Creature* bot = unit->ToCreature())
		if (AB_IS_BOT_UNIT(bot) || AB_IS_BOT_PET_UNIT(bot))
			return AB_GetBotAI(bot)->HasRole(BOT_ROLE_TANK);
...
void bot_ai::BotSpeak(std::string const& text, uint8 msgtype, uint32 language, uint64 speaker, uint64 receiver)
{
    ...
    if (msgtype == CHAT_MSG_WHISPER)
    {
		...
		ASSERT(IS_PLAYER_GUID(((ObjectGuid)receiver)) && "BotSpeak(): whisper receiver is not a player!");

		if (Player* res = ObjectAccessor::FindPlayer((ObjectGuid)receiver))
            res->GetSession()->SendPacket(&data);
    }
    else
    {
		if (Unit* snd = ObjectAccessor::FindUnit((ObjectGuid)speaker))
        {
...
void bot_minion_ai::JustDied(Unit*)
{
    ...
    if (IsTempBot())
    {
		...
		if (!IAmFree())
			AB_RemoveBot(master, me->GetGUID());
...
bool bot_minion_ai::FinishTeleport(/*uint32 mapId, uint32 instanceId, float x, float y, float z, float o*/)
{
    ...
    if (!map || AB_RestrictBots(master, me))
    {
        //ChatHandler ch(master->GetSession());
